...
/// The shipped board, as a peer would load it.
fn map() -> Map {
    Map::shipped().expect("assets/maps loads")
}

/// One peer's version, on a board it has not modified.
fn version(balance: &Balance) -> ProtocolVersion {
    ProtocolVersion::current(balance, &map())
}
...
fn two_peers_on_the_same_tables_agree() {
    let ours = version(&balance());
    let theirs = version(&balance());
    ...
    assert_eq!(
        ours.to_string(),
        format!(
            "v{SCHEMA_REVISION} (balance {:016x}, map {:016x})",
            ours.balance_hash, ours.map_hash
        )
    );
}
...
fn a_balance_change_names_the_balance_and_not_the_schema() {
    let ours = version(&balance());
    let theirs = version(&mutated(|data| {
...
fn both_parts_are_named_when_both_differ() {
    let ours = version(&balance());
    let theirs = version(&mutated(|data| data.creeps[0].hp_mult += 0.5));
    let theirs = ProtocolVersion {
        schema: ours.schema + 1,
        ..theirs
    };
...
fn the_version_tracks_the_rules_and_not_the_match() {
    ...
    let ours = version(&balance());
    let other_match = version(&mutated(|data| data.match_rules.seed ^= 0xdead_beef));
    assert_eq!(
        ours, other_match,
    ...
    let retuned = version(&mutated(|data| data.waves.scaling.hp_base += 1.0));
    assert_ne!(
        ours, retuned,
...
fn reordering_the_files_does_not_move_the_version() {
    ...
    let ours = version(&balance());
    let reordered = version(&mutated(|data| {
        data.towers.reverse();
        data.creeps.reverse();
    }));
